...
6 декабря
- Повторение. RNN (принцип работы, скрытое состояние, размерности) . Векторные представления (embeddings) слов.
...
1. Изучите датасет https://kaggle.com/competitions/store-sales-time-series-forecasting (datasets/store-sales-time-series-forecasting.zip)
    - Какие временняе ряды там присутсвтвуют? Что они представляют?
