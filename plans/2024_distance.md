...
Пример: https://github.com/ivtipm/ML/blob/main/examples/text/text_preprocess.md
...
- Выполнены две работы.
    - Работа 1.
        - EDA ( 7point summary, корреляции, pairplot, гистограмма целевого признака)
        - Масштабирование данных, разделение на Train и Test
        - Применение моделей: дерево решений, лин. регр. + ещё одна любая
        - Оценка моделей и выводы.
        - Комментарии!!!
     - Работа 2.
         - EVA: баланс классов, гистограмма длины текстов,
         - минимальная очистка (стеминг и стоп слова) и Tf-IDF
         - Модели: SVM + любая другая модель
         - Выводы
         - Комментарии!!!
- Собеседование по работам
    1. общая идея обработки данных CRISP-DM, модели, показатели качества моделей (см. вопросы по пунктам выше)
    2. Вопросы по пунктам выше
