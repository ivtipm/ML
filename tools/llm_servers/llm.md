...
Если ollama на линукс запущена как служба, то можно смотреть лог по мере его дополнения:
```bash
journalctl --follow -u ollama.service
```

В логе, помимо прочего, отображается состояние обработки запроса. Например
```
Jul 31 16:02:38 my_computer ollama[41593]: slot print_timing: id  0 | task 5 | prompt processing, n_tokens =   7168, progress = 0.47, t = 243.25 s / 29.47 tokens per second   
```
...
Обработка function_call может быть реализована на сервере, который выступает посредником между пользователем и LLM.
Или же эта обработка может происходить на отдельном специальном сервере (возможно одним из многих). Такие сервера, фактически выполняющие function calling называются MCP серверами. 
...
* Выбирайте модели, которые поместятся в память видеокарты (VRAM) с запасом. 
* Проверяйте экспериментально была ли загружена LLM в VRAM полностью. Ведь в оперативной памяти место занимают другие процессы и также нужно место для хранения данных, с которыми работает LLM. 
* Регулируйте размер контекста. Если он задан слишком большим, то данные могут целиком не поместиться в VRAM и тогда LLM будет работать в разу медленнее. 
* Учитывайте, что размер данных, с которыми работает LLM может быть порядка гигабайт.
