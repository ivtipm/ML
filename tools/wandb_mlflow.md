...
Отедльная функция - автологирование. Автоматически захватывает параметры, метрики, артефакты и модели при использовании стандартных методов фреймворков. Например .fit, .score гиперпараметры моделей и т.п. Поддерживаемые фреимворки: Sklearn, PyTorch, TensorFlow, XGBoost, Hugging Face и др.
...
```bash
mlflow ui
# или с указанием пути к sqlite БД с таблицей записей
mlflow ui --backend-store-uri "sqlite:///my_folder/mlflow.db"
```

Для работы веб сервера mlflow требуется доступ к интернету, без него веб интерфейс будет содержать только несколько полос, обозначающих загрузку таблицы, но сама таблица и веб страница полностью не загрузятся. 

На Windows возможна проблема -- полностью пустая страница вместо веб-интерфйеса.

<details>
Решение: Неверный MIME-тип для .js файлов в реестре Windows (наиболее вероятно)
Это самая частая причина на Windows. Браузер блокирует выполнение скриптов, если они загружаются с MIME-типом text/plain вместо application/javascript 
GitHub
.
✅ Решение: Исправление MIME-типа в реестре
Нажмите Win + R, введите regedit и нажмите Enter
Перейдите по пути:
1
Проверьте параметр Content Type:
❌ Если значение text/plain — это причина проблемы
✅ Измените на application/javascript
Также проверьте и при необходимости исправьте для других расширений:
12
Перезапустите MLflow UI и очистите кэш браузера (Ctrl+F5)
</details>
